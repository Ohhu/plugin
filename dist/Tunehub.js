...
parcelRequire.register("lCxOT", function (module, exports) {
  ...
  $parcel$export(
    module.exports,
    "executeMethodConfigRaw",
    () => executeMethodConfigRaw,
  );
  var $cyXty = parcelRequire("cyXty");
  const delay = (ms11) =>
    new Promise((resolve11) => setTimeout(resolve11, ms11));
  ...
  function calculateSimilarityScore(text11, query11, isSplit11 = false) {
    ...
    if (isSplit11) {
      ...
      for (let i11 = 0; i11 < parts11.length; i11++) {
        if (parts11[i11] === lowerQuery11) return 800 - i11 * 100;
  ...
  async function getMethodConfig(baseUrl11, platform11, functionName11) {
    try {
      ...
      console.error(
        `Get method config error (${platform11}/${functionName11}):`,
        e11,
      );
    }
    return null;
  ...
  function replaceTemplateVariables(template11, variables11) {
    if (typeof template11 === "string") {
      ...
      if (fullMatch11)
        try {
          const func11 = new Function(
            ...Object.keys(variables11),
            `return ${fullMatch11[1]};`,
          );
      ...
      const result11 = {};
      for (const [key11, value11] of Object.entries(template11))
        result11[key11] = replaceTemplateVariables(value11, variables11);
      return result11;
    }
    return template11;
  }
  ...
  async function executeMethodConfigRaw(config11, variables11 = {}) {
    try {
      ...
      const params11 = config11.params
        ? replaceTemplateVariables(config11.params, variables11)
        : undefined;
      const body11 = config11.body
        ? replaceTemplateVariables(config11.body, variables11)
        : undefined;
      const response11 = await (0, $parcel$interopDefault($8zHUo$axios))({
...
parcelRequire.register("cyXty", function (module, exports) {
  $parcel$export(
    module.exports,
    "BASE_URL",
    () => $af8d31735c159a26$export$ca6dda5263526f75,
  );
  $parcel$export(
    module.exports,
    "API_KEY",
    () => $af8d31735c159a26$export$a9861bd62f48e142,
  );
  $parcel$export(
    module.exports,
    "PLATFORM_NAMES",
    () => $af8d31735c159a26$export$ab2a2e5f034797,
  );
  $parcel$export(
    module.exports,
    "QUALITY_MAP",
    () => $af8d31735c159a26$export$174a7998569c8c21,
  );
  $parcel$export(
    module.exports,
    "PAGE_SIZE",
  ...
  const $af8d31735c159a26$export$ca6dda5263526f75 =
    "https://tunehub.sayqz.com/api";
  const $af8d31735c159a26$export$a9861bd62f48e142 =
    "th_dfa1e5bfcc678aed18ee30657d5f260ff5a5be2fb15af6f3";
...
const $99a82f6090a5251e$export$bb9c7f929676dbb6 = async function (query, page) {
  ...
  for (const platform of platforms)
    try {
      const config = await (0, $lCxOT.getMethodConfig)(
        (0, $cyXty.BASE_URL),
        platform,
        "search",
      );
  ...
  const albumList = (0, $lCxOT.sortBySimilarity)(
    Array.from(albumMap.values()),
    query,
    (album) => album.title,
  );
  return { isEnd: true, data: albumList };
};
const $99a82f6090a5251e$export$dc862406499065f2 = async function (
  albumItem,
  page,
) {
  ...
  try {
    ...
    if (!config) return { isEnd: true, musicList: [] };
    const data = await (0, $lCxOT.executeMethodConfig)(config, {
...
const $99a82f6090a5251e$export$4adb7587a1eda30e = async function (
  artistItem,
  page,
  type,
) {
  ...
  try {
    ...
    if (!config) return { isEnd: true, data: [] };
    const data = await (0, $lCxOT.executeMethodConfig)(config, {
      keyword: artistName,
      page: String(page),
      limit: "50",
    ...
    if (data && Array.isArray(data)) {
      ...
      if (type === "music") {
        ...
        return { isEnd: true, data: musicList };
      } else if (type === "album") {
        const albumMap = new Map();
...
const $a4fcabfd0bbb32c7$export$d76128d007d19019 = async function (
  query,
  page,
  type,
) {
  ...
  if (type === "music")
    ...
    const artistList = (0, $lCxOT.sortBySimilarity)(
      Array.from(artistMap.values()),
      query,
      (artist) => artist.name,
      true,
    );
    return { isEnd: true, data: artistList };
  }
  return { isEnd: true, data: [] };
};
const $a4fcabfd0bbb32c7$export$a92854129bc50f89 = async function (
  musicItem,
  quality,
) {
  ...
  if ($a4fcabfd0bbb32c7$var$pendingRequests.has(cacheKey))
    try {
      return await $a4fcabfd0bbb32c7$var$pendingRequests.get(cacheKey);
    } catch (e) {}
    ...
    try {
      ...
      if (response.code === 0 && response.data) {
        ...
        if (Array.isArray(dataArray)) {
          const songData = dataArray.find(
            (item) => String(item.id) === String(musicItem.id),
          );
          if (songData) {
            if (
              $a4fcabfd0bbb32c7$var$lyricCache.size >=
              $a4fcabfd0bbb32c7$var$MAX_CACHE_SIZE
            ) {
              const firstKey = $a4fcabfd0bbb32c7$var$lyricCache
                .keys()
                .next().value;
              if (firstKey) $a4fcabfd0bbb32c7$var$lyricCache.delete(firstKey);
            }
            $a4fcabfd0bbb32c7$var$lyricCache.set(
              cacheKey,
              songData.lyrics || "",
            );
            if (songData.url) return { url: songData.url, quality: quality };
          }
        }
      }
    } catch (e) {
...
const $a4fcabfd0bbb32c7$export$dd8877a67b94ca98 = async function (musicItem) {
  const platform = musicItem.source || "netease";
  const cacheKey = `${platform}_${musicItem.id}`;
  if ($a4fcabfd0bbb32c7$var$lyricCache.has(cacheKey))
    return { rawLrc: $a4fcabfd0bbb32c7$var$lyricCache.get(cacheKey) };
  if ($a4fcabfd0bbb32c7$var$pendingRequests.has(cacheKey))
    try {
  ...
  try {
    const response = await (0, $lCxOT.requestWithRetry)({
      method: "POST",
      url: `${(0, $cyXty.BASE_URL)}/v1/parse`,
      data: { platform: platform, ids: String(musicItem.id), quality: "128k" },
    });
    if (response.code === 0 && response.data) {
      const dataArray = response.data.data;
      if (Array.isArray(dataArray)) {
...
const $a4fcabfd0bbb32c7$export$b0178d0d6466fe81 = async function (topListItem) {
  ...
  try {
    ...
    if (!config) return { ...topListItem, musicList: [] };
    if (platform === "qq") {
    ...
    if (match) {
      ...
      try {
        ...
        if (platform === "netease" && rawData?.result?.tracks)
          return rawData.result.tracks.map((item) => ({
            id: item.id,
            platform: platform,
            source: platform,
            title: item.name,
        ...
        if (platform === "kuwo" && rawData?.musiclist)
          return rawData.musiclist.map((item) => ({
            id: item.id,
            platform: platform,
            source: platform,
            title: item.name,
